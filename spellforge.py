...
def banner():
    """
    Print the Spellforge terminal banner.
...
""")
    ...
    while True:
        ...
        if path.exists():
            ...
            confirm = input(f"  {C.BOLD}Use this existing directory? (y/n): {C.RESET}").strip().lower()
            if confirm != 'y':
            ...
            confirm = input(f"  {C.BOLD}Directory does not exist. Create it? (y/n): {C.RESET}").strip().lower()
            if confirm != 'y':
                info("OK, let's try a different path.")
                continue
...
def ensure_git():
    ...
    if brew_available():
        ...
        if not shutil.which("git"):
            fatal("Git is required but could not be installed automatically. "
...
def install_detect_secrets(pip_bin: str):
    ...
    if brew_result.returncode == 0:
        ...
        warning("brew install succeeded but detect-secrets not found on PATH - falling back to pip.")
    else:
...
# the project guide system expect to find.
# =============================================================================
...
def create_directory_structure(project_path: Path):
    ...
    Create the standard directory structure for a Claude-powered project:
        ...
        docs/
            prd.md        ← placeholder Product Requirements Doc
            project-guide.md          ← placeholder discovery guide
            project-guide-instructions.md  ← instructions for maintaining the guide
            as-built.md   ← placeholder as-built architecture doc
...
def write_settings_local(project_path: Path):
    ...
    if settings_path.exists():
        warning(f"settings.local.json already exists - overwriting with fresh config.")
...
def write_agent_docs(project_path: Path):
    """
    Create the docs/ documentation placeholders that CLAUDE.md references.
    Also copies project-guide-instructions.md so Claude Code knows how to
    maintain the project guide correctly.
    ...
    if not prd_path.exists():
        prd_path.write_text(
            ...
            "> Replace this file with your project's goals, features, and technical requirements.\n"
        )
        success(f"Written placeholder: {prd_path}")
    else:
        warning(f"Already exists - skipping: {prd_path}")

    # ── project-guide.md placeholder ─────────────────────────────────────────
    guide_path = docs_path / "project-guide.md"
    if not guide_path.exists():
        guide_path.write_text(
            "# Project Guide\n\n"
            "> This is the discovery document for existing systems and components.\n"
            "> Keep it updated with every commit. See project-guide-instructions.md for guidance.\n\n"
            "## Directory Structure\n\n"
            "_Document your folder layout here._\n\n"
            "## Systems\n\n"
            "_Document major modules and their entry points here._\n\n"
            "## API Endpoints\n\n"
            "_Document endpoints here as they are added._\n"
        )
        success(f"Written placeholder: {guide_path}")
    else:
        warning(f"Already exists - skipping: {guide_path}")

    # ── project-guide-instructions.md — copy of the real instructions ─────────
    instructions_path = docs_path / "project-guide-instructions.md"
    if not instructions_path.exists():
        # Write the instructions content so Claude Code can find them
        instructions_content = """# Project Guide Maintenance Instructions
...
"""
        ...
        success(f"Written: {instructions_path}")
    else:
        warning(f"Already exists - skipping: {instructions_path}")

    # ── as-built.md placeholder ───────────────────────────────────────────────
    as_built_path = docs_path / "as-built.md"
    if not as_built_path.exists():
        as_built_path.write_text(
            "# As-Built Architecture\n\n"
            "> Document your architecture, validation requirements, error handling standards,\n"
            "> and testing requirements here as the project is built out.\n"
        )
        success(f"Written placeholder: {as_built_path}")
    else:
        warning(f"Already exists - skipping: {as_built_path}")
...
DETECT_SECRETS="{detect_secrets_bin}"
...
# Scan staged files and compare against the baseline
# Any new secrets not in the baseline will cause a non-zero exit
"$DETECT_SECRETS" scan --baseline "$BASELINE" 2>&1
exit_code=$?
...
if [ $exit_code -ne 0 ]; then
  ...
  echo "   If they are false positives, update the baseline:" >&2
  echo "   detect-secrets scan > .secrets.baseline" >&2
  exit 1
fi
...
{C.BOLD}What was set up:{C.RESET}
  ...
  {C.GREEN}✔{C.RESET}  docs/ docs (prd.md, project-guide.md, as-built.md)
...
{C.BOLD}Next steps:{C.RESET}
  ...
  5. Fill in {C.YELLOW}docs/project-guide.md{C.RESET} as you build
...
def get_project_name(project_path: Path) -> str:
    ...
    raw = input(
        f"  {C.BOLD}{C.MAGENTA}➜ Project name [{default_name}]: {C.RESET}"
    ).strip()
...
""")
    ...
    description = input(
        f"  {C.BOLD}{C.MAGENTA}➜ Project description: {C.RESET}"
    ).strip()
...
# File types covered by Prettier (formatter) and ESLint (linter).
# ESLint only lints JS/TS family files - Prettier formats everything.
FRONTEND_FILE_TYPES = [
    # name              ext              prettier  eslint   reason
    ("JavaScript",      ".js",           True,     True,    "Formatting + bug/pattern detection"),
    ("TypeScript",      ".ts",           True,     True,    "Formatting + type-aware lint rules"),
    ("JSX",             ".jsx",          True,     True,    "React components - formatting + hooks rules"),
    ("TSX",             ".tsx",          True,     True,    "TypeScript React - formatting + lint"),
    ("Vue",             ".vue",          True,     True,    "Single-file components - template + script + style"),
    ("HTML",            ".html",         True,     False,   "Markup formatting only - ESLint does not parse HTML"),
    ("CSS",             ".css",          True,     False,   "Property ordering and whitespace"),
    ("SCSS",            ".scss",         True,     False,   "Sass stylesheets"),
    ("Less",            ".less",         True,     False,   "Less stylesheets"),
    ("JSON",            ".json",         True,     False,   "Config file indentation and key ordering"),
    ("YAML",            ".yaml/.yml",    True,     False,   "CI configs, Docker Compose, GitHub Actions"),
    ("Markdown",        ".md",           True,     False,   "Heading spacing, list formatting, line wrapping"),
    ("GraphQL",         ".graphql",      True,     False,   "Query and schema formatting"),
]


def prompt_frontend_tooling(project_path: Path):
    """
    Ask the user whether to install ESLint and/or Prettier for frontend files.
    Presents both tools clearly, explains what each does, and shows the full
    list of supported file types so the user can make an informed choice.

    Both tools require Node.js, already installed at this point for Claude Code.
    ...
    print(f"""
  {C.WHITE}Frontend tooling for non-Python files.{C.RESET}
  {C.BLUE}Ruff already handles all Python linting and formatting.{C.RESET}
  {C.BLUE}These tools extend that coverage to JavaScript, TypeScript, and more.{C.RESET}

  {C.BOLD}The two tools - both are types of linting:{C.RESET}

  {C.GREEN}ESLint  {C.RESET} = Linter    - catches bugs, unsafe patterns, code quality issues
           {C.YELLOW}Covers: .js .ts .jsx .tsx .vue{C.RESET}

  {C.GREEN}Prettier{C.RESET} = Formatter - enforces consistent style (spacing, quotes, line width)
           {C.YELLOW}Covers: all file types listed below{C.RESET}

  {C.BOLD}Full file type coverage:{C.RESET}
""")

    # Print table showing which tool covers which file type
    print(f"  {C.BOLD}{'Type':<16}{'Ext':<14}{'ESLint':<10}{'Prettier':<10}Notes{C.RESET}")
    print(f"  {'─' * 72}")
    for name, ext, prettier, eslint, reason in FRONTEND_FILE_TYPES:
        eslint_col   = f"{C.GREEN}yes{C.RESET}   " if eslint   else f"{C.YELLOW}no{C.RESET}    "
        prettier_col = f"{C.GREEN}yes{C.RESET}   " if prettier else f"{C.YELLOW}no{C.RESET}    "
        print(f"  {name:<16}{ext:<14}{eslint_col}{prettier_col}{reason}")

    print()

    # Ask about each tool separately - user may want one but not the other
    install_eslint = input(
        f"  {C.BOLD}{C.MAGENTA}>>> Install ESLint (linter)? (y/n): {C.RESET}"
    ).strip().lower() == 'y'

    install_prettier = input(
        f"  {C.BOLD}{C.MAGENTA}>>> Install Prettier (formatter)? (y/n): {C.RESET}"
    ).strip().lower() == 'y'

    if not install_eslint and not install_prettier:
        info("Skipping frontend tooling - ruff covers all Python linting and formatting.")
        return

    # ── Install ESLint ────────────────────────────────────────────────────────
    if install_eslint:
        info("Installing ESLint globally via npm...")
        run(["npm", "install", "-g", "eslint"])

        # Flat config format (ESLint v9+) - extend with plugins as the project grows
        eslint_config_path = project_path / "eslint.config.js"
        eslint_config = (
            "// eslint.config.js - ESLint flat config (v9+)\n"
            "// Extend with framework-specific plugins as needed.\n"
            ...
            '      \"no-undef\":          \"error\",\n'
            '      \"no-console\":        \"warn\",\n'
            "      // Enforce readability - mirrors CLAUDE.md coding standards\n"
            '      \"eqeqeq\":            \"error\",\n'
            '      \"prefer-const\":      \"error\",\n'
            '      \"no-var\":            \"error\",\n'
            "    }\n"
        ...
        eslint_config_path.write_text(eslint_config)
    ...
    if install_prettier:
        ...
        # Settings mirror CLAUDE.md coding standards for consistency across
        # Python (ruff) and frontend (prettier) in the same project
        prettierrc_path = project_path / ".prettierrc"
        prettierrc = {
            "tabWidth":       4,
            "useTabs":        True,      # Tabs - matches ruff indent-style
            "singleQuote":    False,     # Double quotes
            "printWidth":     100,       # Matches ruff line-length = 100
        ...
        # Prevent Prettier touching generated or non-source files
        ...
        prettierignore_path.write_text(prettierignore_content)
        success(f"Written: {prettierignore_path}")
        verify_prettier()

    # Remind the user the tools are complementary when both are installed
    if install_eslint and install_prettier:
        info("Both installed - ESLint checks code quality, Prettier handles formatting.")
        info("They complement each other - run both in your CI pipeline.")
...
def verify_packages(project_path: Path):
    ...
    import_names = {
        ...
        "rich":          "rich",
        "loguru":        "loguru",
        "watchdog":      "watchdog",  # only present if user opted in
    }
    ...
    for tool, binary in cli_tools.items():
        if Path(binary).exists():
            ...
            all_passed = True  # non-fatal, pip may have installed it elsewhere
...
def verify_ruff_config(project_path: Path):
    ...
    result   = run([str(ruff_bin), "check", str(project_path), "--select", "E"],
                   capture=True, check=False)
...
def verify_settings_local(project_path: Path):
    ...
    if not any(str(hook_path) in cmd for cmd in hook_commands):
        fatal(f"settings.local.json verification failed - hook command doesn't reference {hook_path}")

    success(f"✔ Verified settings.local.json (valid JSON, hooks + permissions present)")
...
def verify_post_edit_hook(project_path: Path):
    ...
    success(f"✔ Verified post_edit.sh (exists, executable, correct ruff path)")
...
def verify_claude_md(project_path: Path):
    ...
    if missing:
        fatal(
            f"CLAUDE.md verification failed - missing sections:\n"
...
def verify_agent_docs(project_path: Path):
    """
    Confirm all four docs/ documentation files exist.
    ...
    required_files = [
        ...
        project_path / "docs" / "project-guide.md",
        project_path / "docs" / "project-guide-instructions.md",
        project_path / "docs" / "as-built.md",
    ]
...
def verify_precommit_hook(project_path: Path):
    ...
    success(f"✔ Verified pre-commit hook (exists, executable, detect-secrets referenced)")
...
TOOL_MANIFEST = [
    ...
    (
        ...
        "Enforces consistent visual style across all non-Python files. Complements ESLint."
    ),
    (
        "Watchdog",
        False,
...
""")
    ...
    for name, required, category, description, why in TOOL_MANIFEST:
        if not required:
            continue
        print(f"  {C.YELLOW}★{C.RESET} {C.BOLD}{name:<22}{C.RESET} {C.YELLOW}{category:<26}{C.RESET} {why}")
    ...
    for name, required, category, description, why in TOOL_MANIFEST:
        ...
        answer = input(
            f"     {C.BOLD}{C.MAGENTA}>>> Install {name}? (y/n): {C.RESET}"
        ).strip().lower()
        ...
        status = f"{C.GREEN}yes - will install{C.RESET}" if answer == 'y' else f"{C.YELLOW}no - skipping{C.RESET}"
        print(f"     {status}")
        print()
    ...
    for name, required, category, _, _ in TOOL_MANIFEST:
        if required:
            ...
            chosen = (
                (name == "ESLint"    and choices.eslint)   or
                (name == "Prettier"  and choices.prettier) or
                (name == "Watchdog"  and choices.watchdog) or
                (name == "Bandit"    and choices.bandit)
            )
