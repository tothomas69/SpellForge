...
- `do_repair(target_path, rebuild_venv=False)` — Repair flow; runs only Python/venv/base-packages/post-edit-hook steps.
...
- **Repair mode is scope-restricted by design** — `do_repair()` only re-runs the Python/venv/base-packages/post-edit-hook steps. It deliberately does not regenerate project config or scaffolding, so it's safe to run against a project with local edits.
