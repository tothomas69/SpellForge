...
class TestGit:
	...
	def test_git_directory_exists(self):
		...
		assert (PROJECT_ROOT / ".git").is_dir(), ".git directory not found — did git init run?"
...
class TestVirtualEnvironment:
	...
	def test_venv_ruff_exists(self):
		...
		assert ruff.exists(), "ruff binary not found in venv — was it installed by Spellforge?"
...
class TestPyprojectToml:
	...
	def test_pyproject_enforces_coverage_threshold(self):
		"""pyproject.toml must have a coverage threshold and must measure spellforge.py."""
		content = (PROJECT_ROOT / "pyproject.toml").read_text()
		assert "cov-fail-under=" in content, (
			"cov-fail-under not found in pyproject.toml — coverage threshold is missing"
		)
		assert "--cov=spellforge" in content, (
			"--cov=spellforge not found — coverage is not measuring the source file"
		)
	...
	def test_pyproject_has_project_section(self):
		...
		assert "name" in data["project"], "[project] section exists but has no 'name' field"
